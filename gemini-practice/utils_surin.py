...
# 새로고침하기 전에 'chat_history'에 있는 내용 보여주기 
def print_messages():    
...
# metadata에서 체크박스 필터링하기
def search_restaurants(local_jeju_city=False, local_seogwipo_city=False):
    filtered_results = []
...
# 카테고리 분류하기
def category_classification(question):
    # 모델에게 페르소나 부여하기
    ...
    prompt = classification_template.format(question=question)
    response = classification_model.generate_content(prompt,
                                                     generation_config={'response_mime_type':'application/json'}) # JSON으로 추출
...
## 검색형 질문에 대한..
# LLM을 이용한 키워드 추출 함수
def extract_keywords_from_text(question):
    # 모델 페르소나 프롬프트
    ...
    사용자 입력: {question}
    """
    keywords_prompt = keywords_prompt.format(question=question)
    response = extraction_model.generate_content(keywords_prompt,
                                                 generation_config={'response_mime_type':'application/json'}) # JSON으로 추출
...
# 조건으로 정답 찾기
def sorted_df(question, df):
    # keywords_json
    json_data = extract_keywords_from_text(question) 
    ...
    # 4) 비중으로 끝나는 모든 키 필터링 (상위 3개 또는 하위 3개)
    for key, value in json_data.items():
...
# 위의 모든 것 연결하기
def search_chain(question, history, model):
    # 검색형 df 불러오기 
    ...
    괄호 안에 들어있는 숫자는 비중인데, 그 내용과 질문 내용을 포함해서 작성해주세요.
    ...
    return chain.invoke({"history":history, "store":store, "query_string": query_string})
...
def extract_recommendation_keywords_from_text(question):
    ...
    recommendation_prompt = recommendation_keywords_template.format(question=question)

    # LLM에 프롬프트 전송
    response = extraction_model.generate_content(recommendation_prompt,
...
# db에서 필터링하기
def filter_chroma_db(question, chroma_store):
    
    filtered_result = search_restaurants()
    # 질의를 통해 JSON 데이터 추출
    extracted_data = extract_recommendation_keywords_from_text(question)
    
    # 필터링 조건 설정
    filters = \
    {key: extracted_data[key] for key in ["name", "industry", "attraction", "local"] if key in extracted_data}

    if filters:
        # 필터링 실행
        recommendation_filtered_items = \
        [item for item in filtered_result if all(item.get(key) == value for key, value in filters.items())]
    else:
        # 필터링 조건이 없을 경우 유사한 5개의 항목을 검색
        search_results = chroma_store.similarity_search(question, k=5)
        # 검색 결과에서 메타데이터 추출
...
# LLM을 이용한 최종 응답 생성
def recommendation_chain(question, history, model):
    recommendation_store = get_db()
    
    # 데이터베이스에서 관련 정보 검색
    db_results = filter_chroma_db(question, recommendation_store)

    # 검색된 데이터를 LLM 프롬프트에 추가
    db_info = json.dumps(db_results, ensure_ascii=False, indent=2)

    # 채팅 히스토리를 기반으로 프롬프트 생성
    recommendation_chat_template = PromptTemplate.from_template(
    """
    사용자의 질문에 Previous chat history를 참고해 대답해 주세요. 
    사용자 입력: {question}
    ...
    chain = recommendation_chat_template | model | StrOutputParser()
    
    return chain.invoke({'question':question, 'history':history, 'db_info':db_info})

## 기타 질문에 대해..
def other_chain(question, history, model):
    # 프롬프트 생성
    ...
    예시) 사용자 질문: '강남의 유명한 곱창집 추천해줘.'
        답변: '제주도 외의 지역에 대한 추천을 해드릴 수 없습니다.. 제주도 지역에 대한 음식점, 맛집에 대해서 알려드릴 수 있어요!'

    2. 지명, 현지인 비중, 위치, 가게명, 동행자, 방문 목적과 관련된 질문이 아닌 다른 질문이면, 관련된 질문을 해달라고 답변해주세요.
    예시) 사용자 질문: '스트레스 풀고 싶을 때 먹기 좋은 음식점 추천해줘.'
        답변: '해당 질문에 대한 답변은 어렵습니다. 현지인 맛집, 특정 지역의 맛집, 특정 동행자와 많이 먹는 음식에 대한 질문을 해주세요!'

    3. 사용자 질문이 앞뒤가 맞지 않거나 모호한 경우, 아래의 예시처럼 답변해주세요.
    예시) 사용자 질문: '피부 탄력에 좋은 음식점 알려줘'
        답변: '죄송합니다, 입력하신 질문은 이해하기 어려워 추천해 드리기 어렵습니다.
        동행자나 여행 목적에 맞는 제주도 음식점을 추천해 드릴 수 있습니다.
    ...
    # LLM에 프롬프트를 전송하고 결과 받기
    chain = other_prompt | model | StrOutputParser()
    
    return chain.invoke({'question': question, 'history':history})

def main(question, user):
    # 모델 설정
    model = load_model()
    
    # 질문 분류
    classification = category_classification(question)
    ...
    if classification['Classification'] == '검색형':
        print('분류 >> 검색형')
        answer = search_chain(question, history, model)

    elif classification['Classification'] == '추천형':
        print('분류 >> 추천형')
        answer = recommendation_chain(question, history, model)

    else:
        print('분류 >> 기타')
        answer = other_chain(question, history, model)
